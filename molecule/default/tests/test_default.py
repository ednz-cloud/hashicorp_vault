...
def test_vault_interaction(host):
    ...
    vault_operator_init = host.check_output("vault operator init -non-interactive -key-shares=3 -key-threshold=2 -tls-skip-verify -format=json")
    ...
    vault_unseal_0 = host.check_output("vault operator unseal -format=json -tls-skip-verify " + vault_unseal_keys[0])
    vault_unseal_0 = json.loads(vault_unseal_0)
    vault_unseal_1 = host.check_output("vault operator unseal -format=json -tls-skip-verify " + vault_unseal_keys[1])
    vault_unseal_1 = json.loads(vault_unseal_1)
    assert vault_unseal_0['sealed']
    assert not vault_unseal_1['sealed']
    vault_kv_mount = host.check_output("VAULT_TOKEN=" + vault_token + " vault secrets enable -version=1 -tls-skip-verify kv")
